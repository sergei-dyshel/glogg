...
Filter::Filter(const QString& pattern, bool ignoreCase,
    ...
    regexp_( pattern,  patternOptions( ignoreCase ) ),
...
{
  return isCaseInsenstivePattern(regexp_);
}

void Filter::setIgnoreCase( bool ignoreCase )
{
    regexp_.setPatternOptions( patternOptions(ignoreCase) );
}
...
void Filter::saveToStorage( QSettings& settings ) const
{
    ...
    settings.setValue( "ignore_case", isCaseInsenstivePattern( regexp_ ) );
